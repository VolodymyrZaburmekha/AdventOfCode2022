// cr_mn: general feedback
// - I really like this code!  succinct, simple, recursive  :)
// - this code has been formatted after saving the file so I have added file ’settings.json with ‘"editor.formatOnSave": true’ option
// you can try to use it and check whether you like it or not
// - as I told, this kind of exercises I was doing during my ocaml course at the university, here we should be using
// recursion and pattern matching extensively
// - when we meet again I will show you my solution, many of them are 1:1 as yours, I will also remind the concept of "tail recursion"
...
let assertTrue b = if b = false then failwith "assert is wrong" else ()
...
// cr_mn: we can use :: instead of @ while adding one element to the front of list
...
let rec take c l =
    ...
    loop c [] l
// cr_mn: I am sure you will find a lot simpler implementation of 'take' :) (just recursion and pattern marching)
...
// cr_mn: try to implement it yourself without using builtin @ operator
...
// cr_mn: try to use recursion and pattern matching instead of list comprehension :)
...
let rec collect f l =
    ...
    loop [] l
// cr_mn: 'the problem' with this implementation is that many times @ operator is used, calling "a @ b" rewrites the whole "a" list
...
// cr_mn: we can use nice tick with && -> "| head :: tail -> f head && forall f tail"
...
let sequenceEqual f l1 l2 = if length l1 <> length l2 then false else zip f l1 l2 |> forall (fun e -> e = true)
...
let sequenceEqualAlternative f l1 l2 =
    ...
    loop l1 l2
// cr_mn: it's close but this function can be simplified a lot :)

// cr_mn: ... pattern is incomplete maybe because the compiler is not able to handle 'when' clause
let test x =
    match x with
    | y when y = true -> true
